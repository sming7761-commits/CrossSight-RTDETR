...
def load_images(data_yaml, split):
    ...
    if split_path.is_dir():
        imgs = []
        for ext in IMG_EXTS:
            imgs.extend(split_path.rglob(f"*{ext}"))
        return sorted(imgs)
...
def parse_yolo(path, w, h, is_pred=False, conf_thr=0.0, topk=300):
    path = Path(path)
    boxes = []
    if not path.exists():
        return boxes
    ...
    if is_pred:
        boxes = sorted(boxes, key=lambda x: x["conf"], reverse=True)[:topk]
...
def collect(images, pred_root, method, small_area_ratio, iou_thr, conf_thr, topk):
    ...
    for idx, img_path in enumerate(images):
        if idx % 100 == 0:
            print(f"[{method}] {idx}/{len(images)}", flush=True)
        ...
        preds = parse_yolo(pred_path, w, h, is_pred=True, conf_thr=conf_thr, topk=topk)

        small_gts = [g for g in gts if g["area"] / float(w * h + 1e-9) <= small_area_ratio]
        ...
        for gt in small_gts:
            ...
            if best_j >= 0 and best_iou >= iou_thr:
                ...
                norm_err = err / math.sqrt(w * w + h * h)
    ...
    print(f"[{method}] done. matched={matched_small_gt}, total_small_gt={total_small_gt}", flush=True)
    return rows, total_small_gt, matched_small_gt


def summary(rows, total, matched):
    if len(rows) == 0:
        return {
            "small_gt": total,
            "matched": matched,
...
def main():
    ...
    parser.add_argument("--out", default="runs/offset_analysis_fast")
    parser.add_argument("--small-area-ratio", type=float, default=0.01)
    parser.add_argument("--iou-thr", type=float, default=0.5)
    parser.add_argument("--pred-conf-thr", type=float, default=0.05)
    ...
    print(f"[INFO] pred_conf_thr={args.pred_conf_thr}, topk={args.topk}", flush=True)
    ...
    for method, pred_root in configs.items():
        rows, total, matched = collect(
            images, pred_root, method,
            args.small_area_ratio,
            args.iou_thr,
            args.pred_conf_thr,
            args.topk,
        )
